...
**Files:**
- Create: `work/tests/baseline-scenario.md`
...
测试覆盖：空目录创建五份文件；重复运行保留自定义内容；部分存在时仅补齐缺失文件；传入普通文件作为项目根目录时抛出清晰错误。
...
实现只使用 `argparse`、`pathlib`、`shutil` 和 `json`。用 `mkdir(parents=True, exist_ok=True)` 创建目标目录；仅在目标文件不存在时 `copyfile`；输出 UTF-8 JSON 报告；不提供覆盖开关。

- [ ] **Step 4: 验证 GREEN**

Run: `python -m unittest discover -s repo/tests -v`

Expected: 4 tests PASS，无警告。
...
Run: `python C:/Users/redmaple/.codex/skills/.system/skill-creator/scripts/quick_validate.py repo/protect-to-act`
...
Run: `python C:/Users/redmaple/.codex/skills/.system/skill-creator/scripts/quick_validate.py C:/Users/redmaple/.codex/skills/protect-to-act`
